...
@easygrad for_pb_func_test(i1, i2) = begin
	...
	y2=sum(y)
  y2
end debug="test_cases_functions/generated_test.3.9.for_assign.jl"
...
#%%
pb_for_pb_func_test(i1, i2) = begin
	list = 5:105
	s_y = Array{typeof(i1 .+ i2),1}(undef, length(1:length(list)))
	for (_i_i, i) = enumerate(1:length(list))
			s_y[_i_i] = y
			y = i1 .+ i2 .* s_y[_i_i]
	end
	y2 = begin
					(var"##tmp#650", var"##pb#651") = EasyGrad.Zygote.pullback(sum, y)
					var"##tmp#650"
			end
	(y2, EasyGrad.@refclosure((dd_y->begin
									...
									@show typeof(d_y)
									begin
											var"##pb_tmp#652" = var"##pb#651"(d_y2)
											@add! d_y var"##pb_tmp#652"[1]
									end
									(d_i1, d_i2) = (e_zero(i1), e_zero(i2))
									for (_i_i, i) = Iterators.reverse(enumerate(1:length(list)))
										@addt! (d_i1, d_i2, d_y) (d_y[_i_i], y .* d_y[_i_i], i2 .* d_y[_i_i])
									end
									(d_i1, d_i2)
							end)))
end

@show pb_for_pb_func_test(a, b)[2](1.f0)
#%%
