...
# @abstractarray_rule inv(x::AbstractArray) -Ω' * ΔΩ * Ω'
function rrule(::Val{inv}, x::AbstractArray)
    Ω = inv(x)
...
##### `*`
#####
# @abstractarray_rule *(A::AbstractVecOrMat{<:CommutativeMulNumber}, B::AbstractVecOrMat{<:CommutativeMulNumber}) (InplaceableThunk(
#     @thunk(Ȳ * B'),
...
function rrule(::Val{*}, A::AbstractVecOrMat{<:CommutativeMulNumber},
...
function rrule(::Val{*}, A::AbstractVector{<:CommutativeMulNumber},
...
function rrule(::Val{*}, A::CommutativeMulNumber, B::AbstractArray{<:CommutativeMulNumber})
    function times_pullback(Ȳ)
        return (
            NO_FIELDS,
            @thunk(dot(Ȳ, B)'),
...
function rrule(::Val{*}, B::AbstractArray{<:CommutativeMulNumber}, A::CommutativeMulNumber)
    function times_pullback(Ȳ)
        return (
            NO_FIELDS,
            InplaceableThunk(
...
function rrule(::Val{/}, A::AbstractVecOrMat{<:Real}, B::AbstractVecOrMat{<:Real})
...
function rrule(::Val{/}, A::AbstractArray{<:Real}, b::Real)
    Y = A/b
    function slash_pullback(Ȳ)
...
function rrule(::Val{\}, b::Real, A::AbstractArray{<:Real})
    Y = b\A
    function backslash_pullback(Ȳ)
...
function rrule(::Val{-}, x::AbstractArray)
