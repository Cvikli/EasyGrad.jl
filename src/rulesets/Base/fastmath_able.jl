let
    ...
    fastable_ast = quote
        ...
        ## sin
        function rrule(::Val{sin}, x::Number)
            sinx, cosx = sincos(x)
        ...
        ## cos
        function rrule(::Val{cos}, x::Number)
            sinx, cosx = sincos(x)
        ...
        function rrule(::Val{abs}, x::Union{Real, Complex})
            Ω = abs(x)
        ...
        function rrule(::Val{abs2}, z::Union{Real, Complex})
        ...
        ## conj
        function rrule(::Val{conj}, z::Union{Real, Complex})
        ...
        function rrule(::Val{angle}, x::Real)
        ...
        function rrule(::Val{angle}, z::Complex)
            function angle_pullback(ΔΩ)
                x,  y  = reim(z)
                Δu, Δv = reim(ΔΩ)
                # `ifelse` is applied only to denominator to ensure type-stability.
        ...
        function rrule(::Val{hypot}, x::T, y::T) where {T<:Union{Real,Complex}}
            Ω = hypot(x, y)
        ...
        function rrule(::Val{*}, x::Number, y::Number)
