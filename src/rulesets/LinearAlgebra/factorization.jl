...
function rrule(::Val{lu}, A::StridedMatrix, pivot::Union{Val{false},Val{true}}; kwargs...
)
    F = lu(A, pivot; kwargs...)
...
function rrule(::Val{getproperty}, F::TF, x::Symbol) where {T,TF<:LU{T,<:StridedMatrix{T}}}
...
function rrule(::Val{inv}, F::LU{<:Any,<:StridedMatrix})
...
function rrule(::Val{svd}, X::AbstractMatrix{<:Real})
    F = svd(X)
...
function rrule(::Val{getproperty}, F::T, x::Symbol) where T <: SVD
...
function rrule(::Val{eigen}, A::StridedMatrix{T}; kwargs...) where {T<:Union{Real,Complex}}
    F = eigen(A; kwargs...)
...
function rrule(::Val{eigvals}, A::StridedMatrix{T}; kwargs...) where {T<:Union{Real,Complex}}
...
function rrule(::Val{cholesky}, A::Real, uplo::Symbol=:U)
    C = cholesky(A, uplo)
...
function rrule(::Val{cholesky}, A::Diagonal{<:Real}, ::Val{false}; check::Bool=true)
...
function rrule(::Val{cholesky}, A::LinearAlgebra.HermOrSym{<:LinearAlgebra.BlasReal, <:StridedMatrix},
    ::Val{false};
    check::Bool=true,
)
    C = cholesky(A, Val(false); check=check)
...
function rrule(::Val{cholesky}, A::StridedMatrix{<:LinearAlgebra.BlasReal},
    ::Val{false};
    check::Bool=true,
)
    C = cholesky(A, Val(false); check=check)
...
function rrule(::Val{getproperty}, F::T, x::Symbol) where {T <: Cholesky}
