...
function rrule(::Val{/}, A::AbstractMatrix{<:Real}, B::T) where T<:SquareMatrix{<:Real}
    Y = A / B
...
function rrule(::Val{\}, A::T, B::AbstractVecOrMat{<:Real}) where T<:SquareMatrix{<:Real}
    Y = A \ B
...
function rrule(::Val{diag}, A::AbstractMatrix)
...
if VERSION ≥ v"1.3"
    function rrule(::Val{diag}, A::AbstractMatrix, k::Integer)
        function diag_pullback(ȳ)
    ...
    function rrule(::Val{diagm}, m::Integer, n::Integer, kv::Pair{<:Integer,<:AbstractVector}...)
...
function rrule(::Val{diagm}, kv::Pair{<:Integer,<:AbstractVector}...)
    function diagm_pullback(ȳ)
...
function rrule(::Val{*}, D::Diagonal{<:Real}, V::AbstractVector{<:Real})
...
function rrule(::Val{adjoint}, A::AbstractMatrix{<:Number})
    adjoint_pullback(ȳ::Composite) = (NO_FIELDS, ȳ.parent)
    adjoint_pullback(ȳ::AbstractVecOrMat) = (NO_FIELDS, adjoint(ȳ))
    return adjoint(A), adjoint_pullback
end

function rrule(::Val{adjoint}, A::AbstractVector{<:Number})
    adjoint_pullback(ȳ::Composite) = (NO_FIELDS, vec(ȳ.parent))
    adjoint_pullback(ȳ::AbstractMatrix) = (NO_FIELDS, vec(adjoint(ȳ)))
    return adjoint(A), adjoint_pullback
end
...
function rrule(::Val{transpose}, A::AbstractMatrix{<:Number})
    transpose_pullback(ȳ::Composite) = (NO_FIELDS, ȳ.parent)
    transpose_pullback(ȳ::AbstractVecOrMat) = (NO_FIELDS, transpose(ȳ))
    return transpose(A), transpose_pullback
end

function rrule(::Val{transpose}, A::AbstractVector{<:Number})
    transpose_pullback(ȳ::Composite) = (NO_FIELDS, vec(ȳ.parent))
    transpose_pullback(ȳ::AbstractMatrix) = (NO_FIELDS, vec(transpose(ȳ)))
    return transpose(A), transpose_pullback
end
...
function rrule(::Val{det}, X::Union{Diagonal, AbstractTriangular})
    y = det(X)
...
function rrule(::Val{logdet}, X::Union{Diagonal, AbstractTriangular})
    y = logdet(X)
