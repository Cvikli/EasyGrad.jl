...
rrule(::Val{BLAS.dot}, x, y) = rrule(dot, x, y)

function rrule(::Val{BLAS.dot}, n, X, incx, Y, incy)
...
function rrule(::Val{BLAS.nrm2}, x)
    Ω = BLAS.nrm2(x)
...
function rrule(::Val{BLAS.nrm2}, n, X, incx)
...
function rrule(::Val{BLAS.asum}, x)
...
function rrule(::Val{BLAS.asum}, n, X, incx)
...
function rrule(::Val{gemv}, tA::Char, α::T, A::AbstractMatrix{T},
...
function rrule(::Val{gemv}, tA::Char, A::AbstractMatrix{T}, x::AbstractVector{T}
) where T<:BlasFloat
...
function rrule(::Val{gemm}, tA::Char, tB::Char, α::T, A::AbstractMatrix{T}, B::AbstractMatrix{T}
) where T<:BlasFloat
    C = gemm(tA, tB, α, A, B)
...
function rrule(::Val{gemm}, tA::Char, tB::Char, A::AbstractMatrix{T}, B::AbstractMatrix{T}
) where T<:BlasFloat
